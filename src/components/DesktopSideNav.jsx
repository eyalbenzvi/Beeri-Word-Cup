...
import { Home as HomeIcon, ClipboardList, Trophy, Goal, BarChart3, Settings } from "lucide-react";
    ...
    >
      ...
      {user && (
        <button
          ...
          className={`mt-6 flex items-center gap-3 px-3 py-2.5 rounded-xl border-none cursor-pointer bg-transparent text-right transition-colors ${
            currentPage === "profile" ? "bg-primary/10" : "hover:bg-bg-soft"
          }`}
          aria-label="פרופיל"
        >
          <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-sm font-extrabold text-white border-2 border-primary-dark flex-shrink-0">

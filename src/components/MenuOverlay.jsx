...
function MenuSection({ icon, title, active, onToggle, children }) {
  return (
    <div className="bg-white rounded-2xl border-2 border-border overflow-hidden">
      <button
        onClick={onToggle}
        className={`w-full flex items-center justify-between p-4 bg-transparent border-none cursor-pointer text-right transition-colors ${active ? "bg-primary/5" : "hover:bg-bg-soft"}`}
      >
      ...
      <div id="menu-overlay" role="dialog" aria-modal="true" aria-label="תפריט מידע" className="fixed top-0 right-0 h-full w-[85%] max-w-sm bg-bg z-[70] shadow-2xl overflow-y-auto animate-slide-in pb-[env(safe-area-inset-bottom)]">
        <div className="bg-primary text-white p-5 flex items-center justify-between border-b-4 border-primary-dark">
          <button
            ...
            className="text-white/90 bg-transparent border-none cursor-pointer p-0 leading-none hover:text-white"
          >
            <X size={24} />
          </button>
          ...
          >
            <div className="text-right text-sm text-ink-muted space-y-2 leading-relaxed">
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 mt-3 text-white font-extrabold py-3 px-4 rounded-2xl no-underline text-sm transition"
                style={{ background: "var(--color-whatsapp)", boxShadow: "0 4px 0 0 var(--color-whatsapp-dark)" }}
              >
